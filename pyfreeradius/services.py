            ...
            for usergroup in user.groups:
                ...
                if group and not (group.checks or group.replies or len(group.users) > 1):
                    raise ServiceExceptions.GroupWouldBeDeleted(
                        f"Group '{group.groupname}' would be deleted as it has no attributes and only one user: "
            ...
            for groupuser in group.users:
                ...
                if user and not (user.checks or user.replies or len(user.groups) > 1):
                    raise ServiceExceptions.UserWouldBeDeleted(
                        f"User '{user.username}' would be deleted as it has no attributes and only one group: "
